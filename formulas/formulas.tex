...
\begin{itemize}
    ...
    \begin{itemize}
        \item Paralelo: $k_e = k_1 + k_2 + \dots$ y en serie: $\frac{1}{k_e} = \frac{1}{k_1} + \frac{1}{k_2} + \dots$
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Nueva versión de la 2da Ley de Newton:} $\sum \vec{F} = ( d\vec{P}) / dt$.
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Resistencia ($R$):} $R = \Delta V / I$ , Ley de Ohm: $\Delta V = IR$
...
\begin{itemize}
    ...
    $$\frac{F_B}{\ell} = \frac{\mu_0 I_1 I_2}{2\pi a}$$ (\(I_1\),\(I_2\) corrientes en los alambres y \(a\) es la distancia entre ellos).
