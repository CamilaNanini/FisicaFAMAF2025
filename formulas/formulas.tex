...
\begin{itemize}
    \item \includegraphics[width=0.4\textwidth]{Trigonometria.png}
    \item \textbf{Conversión de Ángulos:}
    $180^\circ = \pi \text{ Radianes}$

    \item \textbf{Componentes de un Vector:}$A_x = A \cos \theta , A_y = A \sin \theta$

    \item \textbf{Módulo de un Vector:}
...
\begin{itemize}
    ...
    \item \textbf{Velocidad Media:} $v_{med} = \frac{x_f - x_i}{t_f - t_i} = \frac{\Delta x}{\Delta t}$
    \item \textbf{Aceleración Promedio:} $a_{prom} = \frac{\Delta v_x}{\Delta t} = \frac{v_{xf} - v_{xi}}{t_f - t_i}$
    \item \textbf{Movimiento Rectilíneo Uniforme (MRU):}
    $a = 0 \implies v = \text{constante} \implies x(t) = x_0 + v \cdot t$
    \item \textbf{Movimiento Rectilíneo Uniformemente Variado (MRUV):}
    $a = \text{constante} \implies v(t) = v_0 + a \cdot t \implies x(t) = x_0 + v_0 t + \frac{1}{2} a t^2$
    \item \textbf{Movimiento Circular:}
    \begin{itemize}
        \item Período ($T$): Tiempo por vuelta. $T = \frac{2\pi R}{v}$
        \item Frecuencia ($f$): $f = 1/T$ (Hertz, Hz).
        \item Aceleración centrípeta: $a_c = \frac{v^2}{r} = \omega^2 r$
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Paralelo: $k_e = k_1 + k_2 + \dots$
        \item Serie: $\frac{1}{k_e} = \frac{1}{k_1} + \frac{1}{k_2} + \dots$
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Trabajo ($W$):} $W = F \Delta r \cos(\theta)$
    \item \textbf{Energía Cinética ($K$):} $K = \frac{1}{2} m v^2$
    ...
    \begin{itemize}
        ...
        \item Energía Potencial Elástica: $U_e = \frac{1}{2} k \Delta x^2$
    \end{itemize}
    \item \textbf{Energía Mecánica ($E$):} $E = K + U$
    \begin{itemize}
        \item Si solo hay fuerzas conservativas: $\Delta E = 0$
        \item Con fuerzas no conservativas: $\Delta E = W_{NC}$
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Momento Lineal o Cantidad de Movimiento Lineal:} $\vec{P} = m\vec{v}$.
    \item \textbf{Nueva versión de la 2da Ley de Newton:} $\sum \vec{F} = \frac{d\vec{P}}{dt}$.
    ...
    \begin{itemize}
        \item Después del choque las partes quedan unidas en un solo cuerpo y $K_f < K_i$ (Ej:plastilina contra caja)
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \(k_e = \dfrac{1}{4\pi\varepsilon_0} \approx 8{,}99 \times 10^9 \, \text{N}\cdot\text{m}^2/\text{C}^2\)
        \item \(\varepsilon_0 \approx 8{,}85 \times 10^{-12} \, \text{C}^2/(\text{N}\cdot\text{m}^2)\)
    \end{itemize}
    ...
    \]
    (campo eléctrico generado por una carga \(q\), donde \(\hat{r}\) apunta desde la carga hacia el punto donde se evalúa el campo).
...
o sea que el caso de una carga puntual en el campo la energía potencial U = $q_0Ey$ donde y es la distancia vertical.
...
\begin{itemize}
    \item \textbf{Capacitancia ($C$):} $C = \frac{Q}{\Delta V}$
    \item \textbf{Capacitancia de un capacitor de placas paralelas:} $C = \epsilon_0 \frac{A}{d}$
    \item \textbf{Capacitancia de un capacitor lleno de un material con constante dieléctrica $\kappa$:} 
    $C = \frac{Q_0}{\Delta V} = \kappa \frac{Q_0}{\Delta V_0} $
    \item \textbf{Energía almacenada en un capacitor:} $U = \frac{1}{2} C V^2$
    ...
    \end{itemize}
    \item \textbf{Resistencia ($R$):} $R = \frac{\Delta V}{I}$ , Ley de Ohm: $\Delta V = IR$
    ...
    \end{itemize}
    \item La potencia disipada en un resistor: $P = I^2 R = \frac{\Delta V^2}{R}$
...
\section{Magnetismo}
\begin{itemize}
    \item La fuerza que actúa sobre una carga \(q\) que se mueve con velocidad \(\vec{v}\) en un campo magnético \(\vec{B}\) está dada por la expresión: $\vec{F} = q\,\vec{v} \times \vec{B}$
    \item theta es el ángulo entre v y B. Por esta expresión puede que F sea igual a cero cuando v es paralela o antiparalela a B (0 o 180°) y es máxima cuando v es perpendicular a B (90°). $F = |q| v B \sen \theta$
    \item Cuando una partícula cargada se mueve en una región donde existen campos eléctrico \(\vec{E}\) y magnético \(\vec{B}\), la fuerza total que experimenta está dada por: $\vec{F} = q\left(\vec{E} + \vec{v} \times \vec{B}\right)$
    \item El flujo magnético a través de una superficie se define como: $\Phi_B = \int B_\perp\, dA = \int B \cos\phi\, dA = \int \vec{B}\cdot d\vec{A}$. El flujo magnético total a través de una superficie cerrada siempre es igual a cero. 
    \item La aceleración centrípeta es \(a_c = \dfrac{v^2}{R}\) y la única fuerza que actúa es la fuerza magnética. De acuerdo con la segunda ley de Newton: $|\vec{F}| = |q|\,vB = m\,\frac{v^2}{R}$
    \item (\textit{Radio de una órbita circular en un campo magnético}) El radio de la trayectoria es: $R = \frac{m v}{|q| B}$
    \item La rapidez angular \(\omega\) de la partícula se obtiene a partir de la relación: $\omega = \frac{v}{R}$
    \item (\textit{Frecuencia del ciclotrón}) La frecuencia de la partícula es: $f = \frac{\omega}{2\pi}$
    ...
    \item \textit{momento dipolar magnético} o \textit{momento magnético} de la espira: $\vec{\mu} = I\,\vec{A}$ . Con N espiras de la misma área $\vec{\mu}_{bobina} = N I\,\vec{A}$
    \item La energía potencial de un dipolo magnético en un campo magnético está dada por: $U = -\vec{\mu}\cdot\vec{B} = -\mu B\cos\phi$
    \item
...
\begin{itemize}
    ...
    entre ellos. Dos objetos están en \textbf{equilibrio térmico} si están en contacto térmico y no hay
    intercambio neto de energía entre ellos.
    \item \textbf{Ley cero de la termodinámica:} Si los objetos A y B están por separado en equilibrio térmico con un tercer objeto C,
    ...
    \item \textbf{Expansión Volumétrica:} $ \Delta V = \beta V_0 \Delta T $. Donde $\beta$ es el coeficiente de expansión volumétrica y $\beta = 3\alpha$.
    ...
    \item Para una masa constante de gas ideal, el producto $nR$ es constante, por lo que la cantidad $\dfrac{pV}{T}$ también permanece constante. Si los subíndices $1$ y $2$ representan dos estados distintos del mismo gas, se cumple:
    $\frac{p_1 V_1}{T_1} = \frac{p_2 V_2}{T_2} = \text{cte}$
    \item Procesos: isotérmicos = T ctate , isobéricos = P ctate , isocóricos = V ctate, procesos adiabáticos = aislados térmicamente.
    \item \textbf{Número de Avogadro ($N_A$):} Representa la cantidad de moléculas en un mol: $N_A = 6.022 \times 10^{23} \, \text{mol}^{-1}$
    ...
    \item Convenciones: El calor es positivo cuando entra al sistema y negativo cuando sale del sistema. El trabajo es positivo cuando el sistema realiza trabajo sobre el entorno y negativo cuando el entorno realiza trabajo sobre el sistema.
    \item En un cambio finito de volumen desde $V_1$ hasta $V_2$, el trabajo realizado está dado por
    $W = \int_{V_1}^{V_2} p\, dV$
    \item \textbf{En un proceso isobárico:} El trabajo realizado por el gas se calcula como $W = \int_{V_1}^{V_2} p\, dV = p\,(V_2 - V_1)$
