...
\begin{itemize}
    ...
    \includegraphics[width=0.3\textwidth]{procruz.png}
    \item $180^\circ = \pi \text{ Rad}$ \textbf{Componentes de un Vector:}$A_x = A \cos \theta , A_y = A \sin \theta$

    \item $|\vec{A}| = \sqrt{A_x^2 + A_y^2}$ , $\vec{A} \cdot \vec{B} = |\vec{A}||\vec{B}| \cos(\alpha)$
\end{itemize}
