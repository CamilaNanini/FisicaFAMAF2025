...
\begin{itemize}
    ...
    \begin{itemize}
        \item \(k_e = 1 / (4\pi\varepsilon_0) \approx 8{,}99 \times 10^9 \, \text{N}\cdot\text{m}^2/\text{C}^2\) y \(\varepsilon_0 \approx 8{,}85 \times 10^{-12} \, \text{C}^2/(\text{N}\cdot\text{m}^2)\)
    \end{itemize}
    ...
    \begin{itemize}
        \item Interior (\(r<R\)):
        $Q_{\text{enc}} = Q r^3/R^3
        , E(r)= [Q / (4\pi\varepsilon_0 R^3)]r$

        \item Exterior (\(r\ge R\)):
        $E(r)=Q / (4\pi\varepsilon_0 r^2)$
    \end{itemize}

    \item \textbf{Caso: Esfera conductora cargada}

    \begin{itemize}
        \item Interior (\(r<R\)): $Q_{\text{enc}}=0 \quad\Rightarrow\quad E=0$

        \item Exterior (\(r\ge R\)):
        $E(4\pi r^2)=Q/\varepsilon_0 \quad\Rightarrow\quad
        E(r)=Q / (4\pi\varepsilon_0 r^2)$
    \end{itemize}

    \item \textbf{Caso: Carga lineal infinita}
     $Q_{\text{enc}}=\lambda L$ $\quad E(r)=\lambda/(2\pi\varepsilon_0 r)$

    \item \textbf{Caso: Lámina infinita cargada}
    $Q_{\text{enc}}=\sigma A$ $\Rightarrow$ $\quad E=\sigma/(2\varepsilon_0)$ 
\end{itemize}
...
\item El trabajo realizado por el campo eléctrico sobre una carga \(q\) al atravesar una diferencia de potencial \(V\) es $W = qV.$

\item La presencia de una diferencia de potencial implica la existencia de un campo eléctrico, ya que el campo se define como el gradiente del potencial.
...
\item Trabajo necesario para ensamblar un sist d cargas, trayéndolas desde el infi hasta sus posiciones:
$W_{\text{ext}} = U$. Se introducen las cargas una a una: $$W = \sum_{i=1}^{N} q_i V_i$$ 
donde el potencial en la posición de la carga $q_i$ es:
$$V_i = \sum_{j<i} k \frac{q_j}{r_{ij}}$$ y $(r_{ij})$ es la distancia entre las cargas $q_i$ y $q_j$.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Cilindro macizo largo (radio $R$, corriente uniforme $I$):}
    Por simetría $\vec B$ es tangencial y depende solo de $r$.
    \[B(r)=
    \begin{cases}
        \mu_0 I r /(2\pi R^2), & r<R \\[4pt]
        \mu_0 I /(2\pi r), & r>R
    \end{cases}
    \]
    \item \textbf{Cilindro hueco largo (radios $a,b$, corriente uniforme $I$):}
    Densidad $J=I/[\pi(b^2-a^2)]$.
    \[ B(r)=
    \begin{cases}
        0, & r<a \\[4pt]
        \mu_0 I /(2\pi r)\,(r^2-a^2)/(b^2-a^2), & a<r<b \\[4pt]
        \mu_0 I /(2\pi r), & r>b
    \end{cases}
    \]
\end{itemize}
...
\begin{itemize}
    ...
    $ \Delta U = Q - W $
    \item Trabajo isotérmico: $W = nRT \ln \left( V_f/V_i \right)$
